...
&emsp;&emsp;无论您采用后文中的何种方式来编译C1enOS，您必须先按照本小节中的步骤，初始化您的开发环境。
...
**请留意，若您的Linux系统是在虚拟机中运行的，还请您在您的VMware/Virtual Box虚拟机的处理器设置选项卡中，开启Intel VT-x或AMD-V选项，否则，C1enOS将无法运行。**
...
&emsp;&emsp;为减轻配置环境的负担，C1enOS即将发布一个Docker编译环境，便于开发者运行C1enOS。我们强烈建议您采用这种方式来运行C1enOS。

&emsp;&emsp;本节假设以下操作均在Linux下进行。

### 获取C1enOS编译镜像

&emsp;&emsp;当您成功安装了docker之后，您可以通过以下命令，下载C1enOS的编译镜像：

```shell
docker pull c1enos/c1en-v1.1.1
```
...
&emsp;&emsp;如果不出意外的话，这将是运行C1enOS的最后一步。您只需要在C1enOS的根目录下方，执行以下命令，即可运行C1enOS。

```shell
make run-docker
```

&emsp;&emsp;稍等片刻，C1enOS将会被运行。
...
&emsp;&emsp;**如果显示的版本号为2.06及以上，且您已经按照第一小节中的内容，安装相关的依赖，那么，恭喜您，您可以直接在本机编译C1enOS!**
